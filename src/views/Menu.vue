...
<ul class="nav nav-tabs flex items-center justify-center space-x-2 border-0 mb-4" id="myTab" role="tablist">
  <li v-for="(category,index) in categories" :key="index" class="nav-item " role="presentation">
    <button class="nav-link text-white w-full h-full" id="home-tab" data-bs-toggle="tab" :data-bs-target="`#${category.name}`" type="button" role="tab" aria-controls="home" aria-selected="true">{{ category.name }}</button>
  </li>
</ul>
